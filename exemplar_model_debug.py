...
class Agent:
    ...
    def produce(self,wc_index):
        ...
        self.p_vec_word = str(self.p_vec_w(prod_target, wc_index))
        self.p_vec_seg = str(self.p_vec_s(prod_target))
        self.p_vec = str(p_vector)
        
        #p_noise = self.default_b(p_vector)
        p_noise = self.wedel_b(p_vector)
        self.p_vec_noise = str(p_noise)
        #p_noise = p_vector
    ...
    def plot_space(self,cycle,act_alpha=False): 
        fig, ax = plt.subplots()
        plt.xlim([0, 100])
        plt.ylim([0, 100])
        z = self.activations
        ...
        name = self.name+'_Cycle_'+str(cycle)
        plt.title(name)
        plt.savefig(name+'.png')
        
        
        
def pp_loop(iterations,monitor_cycles,agent1,agent2):
    for i in range(iterations):
        
        if i in monitor_cycles:
            out_file = open('example_output.txt','a',encoding='utf-8')
            out_file.write('CYCLE '+str(i)+'\n\n')
            out_file.write(s1+'\n'+s2+'\n'+s1+'\n\n')
            out_file.write(str(agent1.space)+'\n')
        
            # Agent 1 produces, agent 2 receives
            agent2.receive(agent1.produce(0))
            out_file.write('\n #### Word cat 1 ####\n')
            out_file.write('\n p_vec_word '+agent1.p_vec_word+'\n')
            out_file.write('\n p_vec_seg '+agent1.p_vec_seg+'\n')
            out_file.write('\n p_vec_noise'+agent1.p_vec_noise+'\n'*4)
...
s1 = '#'*23
s2 = '#'*4+' Agent 1 space '+'#'*4
        
if __name__=='__main__':
    tot_cycles = 151
    monitor_cycles = [0,50,100,150]
    ...
    pp_loop(tot_cycles,monitor_cycles,agent1,agent2)
